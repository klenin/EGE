...
sub image_size {
    ...
    my $i = rnd->in_range(1, 6);
    my $v = $x * $y * $i;
    my $n = 2 ** $i;
    $self->{text} = "Для хранения растрового изображения размером $x x $y пикселей отвели $v байт памяти. Каково максимально возможное число цветов в палитре изображения?";
    $self->variants($n, $n * 2 - 1, $n*2, $n / 2);
}
