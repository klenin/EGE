...
sub _plane_divide {
    my $n = @_;
    $n == 0 ? 1 : _plane_divide($n - 1) + $n;
}
sub plane_divide {
    my ($self) = @_;
    my $n = rnd->in_range(5, 10);
    my $n_text = num_by_words($n, 0, 'instrumental');
    ...
    $self->{text} =
        ...
        "<i><b>L(0)=1</b></i><br />" .
        "Каково максимальное число областей, на которые плоскость делится $n_text прямыми?";
    $self->{correct} = $n*($n-1)/2 + 1;
}
...
sub hanoi {
    ...
    $self->{text} =
        ...
        "<i><b>S(1)=1</b></i><br />" .
        "Чему равно значение функции <i><b>S($n)</b></i>? В ответ запишите только натуральное число.";
    $self->{correct} = 2**$n - 1;
}
...
sub fib {
    ...
    $self->{text} =
        ...
        "Чему равно $word{$n} число в последовательности  Фибоначчи?";
    $self->{correct} = _fib($n);
}
