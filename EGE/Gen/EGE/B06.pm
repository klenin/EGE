...
sub plane_divide {
    ...
    my $num = rnd->pick(5 .. 10);

    
    $self->{text} =
        "Максимальное число <i><b>L(n)</b></i> областей, на которые плоскость делится n прямыми," .
        "можно вычислить  с помощью рекуррентного соотношения:<br />" .
        "<i><b>L(n)=L(n−1)+n</b></i> при натуральных <i><b>n≥1</b></i><br />" .
        "<i><b>L(0)=1</b></i><br />" .
        "Каково максимальное число областей, на которые плоскость делится $names{$num} прямыми?";

    $self->{correct} = _divide($num);
}
...
sub hanoi {
    ...
    $self->{text} =
        "Для подсчета минимального числа ходов в головоломке ханойская башня используется функция <i><b>S(n)</b></i>," .
        "которая вычисляется по следующему алгоритму:<br />" .
        "<i><b>S(n)=2*S(n−1)+1</b></i> при натуральном <i><b>n>1</b></i><br />" .
        "<i><b>S(1)=1</b></i><br />" .
        "Чему равно значение функции <i><b>S($num)</b></i>? В ответ запишите только натуральное число.";

    $self->{correct} = _hanoi($num);
}
...
sub fib {
    ...
    $self->{text} =
        "Последовательность чисел Фибоначчи задается рекурентным соотношением:<br />" .
        "<i><b>F(1) = 1</b></i><br />" .
        "<i><b>F(2) = 1</b></i><br />" .
        "<i><b>F(n) = F(n-2) + F(n-1)</b></i>, при  <i><b>n > 2</b></i>, где <i><b>n</b></i> - натуральное число.<br />" .
        "Чему равно $names{$num} число в последовательности  Фибоначчи?";

    $self->{correct} = _fib($num);
}
