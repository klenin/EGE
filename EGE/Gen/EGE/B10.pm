...
sub min_period_of_time {
    ...
    my @female_names = qw(Кат Тан Даш и ей);
    my @male_names = qw(Серг Андр Алекс ея ей ею еем);
    my $f_name = $female_names[rnd->in_range(0,2)];
    my $m_name = $male_names[rnd->in_range(0,2)];

    $self->{text} =
        "У $f_name$female_names[3] есть доступ к сети Интернет по высокоскоростному одностороннему радиоканалу,
        обеспечивающему скорость получения информации 2<sup>$high_speed</sup> бит в секунду.
        У $m_name$male_names[3] нет скоростного доступа в Интернет, но есть возможность получать информацию
        от $f_name$female_names[3] по телефонному каналу со средней скоростью 2<sup>$slow_speed</sup> бит в секунду.
        $m_name$male_names[4] договорился с $f_name$female_names[4], что она скачает для него данные объемом $required_data
         Мбайт по высокоскоростному каналу и ретранслирует их $m_name$male_names[5] по низкоскоростному каналу.<br/>
        <br/>Компьютер $f_name$female_names[3] может начать ретрансляцию данных не раньше, чем им будут получены
        первые $full_data Кбайт этих данных. Каков минимально возможный промежуток времени
        (в секундах) с момента начала скачивания $f_name$female_names[4] данных до полного их получения
        $m_name$male_names[6]?<br/><br/>В ответе укажите только число, слово «секунд» или букву «с» добавлять не нужно.";
