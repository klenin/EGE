...
sub get_n {
    ...
    my $bit = EGE::Bits->new();
    $bit->set_size(int(log($ask+1) / log(2))+1);
    $bit->set_dec($ask+1);

    if (EGE::Bits::get_bit($bit, 0) == 1) { $bit->inc_w_resize(); };
    my $size = $bit ->get_size();
    while (1){
        my $unit_count=0;   
        for (my $i = 2; $i < $size; $i++){
            if (EGE::Bits::get_bit($bit, $i)) { $unit_count++; };
        };
        if (EGE::Bits::get_bit($bit, 1) ==($unit_count % 2)) { 
            last;
        };
            for my $i (1..2) {  $bit->inc_w_resize(); };
    }
    $bit->shift_(2);
    $self->{text} = 
