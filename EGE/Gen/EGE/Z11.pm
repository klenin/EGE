...
sub gen_branches_sum {
    ...
    my $sum = [ '+', gen_branches_sum($n - 1, $branches), $cur_oper ];
    $sum;
}
...
sub recursive_alg {
    ...
    my @args = ( param => $param, value => $task->{val}, sign => $alg->{sign},
                 threshold => $alg->{threshold}, branches_call => \@branches_call,
                 branches_sum => $branches_sum, ret => $task->{ret}, start => $alg->{start}, print_type => $task->{print_type} );
    $self->{text} =
