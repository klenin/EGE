...
sub tumblers
{
    ...
    $self->{text} ="Выбор ре­жи­ма ра­бо­ты в не­ко­то­ром устрой­стве осу­ществ­ля­ет­ся ус­та­нов­кой ручек $tumblers_count тум­бле­ров, ".
...
sub tumblers_min
{
    ...
    $ans = int(log($n) / log($tumbler_state) + 0.9999);
    $self->{correct} = $ans;
}
1;