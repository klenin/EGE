...
sub dif_message {
    my ($self) = @_;
    my $n = rnd->in_range(2, 9);
    my $time = rnd->in_range(2, 7);
    my $all_time = $n * $time;
    my $n_flags = rnd->in_range(2, 6);
    my $m = $n - 1;
    $self->{text} ="В детскую игрушку «Набор юного шпиона» входят два одинаковых комплекта из $n_flags флажков различных цветов.
         Сколько различных тайных сообщений можно передать этими флажками, условившись менять выставленный флажок каждые 
         $time минут(ы) и наблюдая за процессом $all_time минут(ы)? Наблюдатель видит вынос первого флажка и $m перемен(ы\у) флажка. 
