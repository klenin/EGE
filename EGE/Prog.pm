...
sub polinom_degree {
    ...
    elsif ($self->{op} eq '+' || $self->{op} eq '-') { max(map($self->{$_}->polinom_degree($var), $self->_children)) } 
...
sub polinom_degree { $_[1] eq $_[0]->{name} }
...
sub complexity { my $self = shift; max(map($_->complexity(@_), @{$self->{statements}})) }
